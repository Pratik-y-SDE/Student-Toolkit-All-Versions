...
def show_countdown(seconds):
    """Print a visible countdown so the user knows the time limit."""
    print(f"  [TIMER] Time limit: {seconds} seconds. Answer quickly!")
    print(f"  (The timer is advisory — answer and press Enter to continue.)")
...
def run_quiz(topic, questions, timed=False):
    ...
    for i, item in enumerate(questions, 1):
        ...
        # Simple reliable input loop — works in VS Code, CMD, and all terminals
        ans = ''
        q_start = time.time()
        while True:
            ...
            if timed and (time.time() - q_start) > 20:
                print("  [TIME UP] You took too long! Answer not counted.")
                ans = ''
                break
        ...
        if ans == item['answer']:
            ...
            print(f"  [SKIP]  Skipped. Correct answer: {item['answer']}")
            review.append((item['q'], "Skipped", item['answer'], False, item['explanation']))
        else:
            print(f"  [WRONG] Wrong! Correct answer: {item['answer']}")
